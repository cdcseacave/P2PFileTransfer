...
pub async fn handle_resume(transfer_id: String, to: String, path: PathBuf) -> Result<()> {
    ...
    if !path.exists() || !path.is_dir() {
        anyhow::bail!("Folder path does not exist or is not a directory: {}", path.display());
    }
    
    // Load state from file
    let state_path = PathBuf::from(format!("transfer_{}.json", transfer_id));
    if !state_path.exists() {
        anyhow::bail!("State file not found: {}. Transfer may have already completed.", state_path.display());
    }
    ...
    let mut session = FolderTransferSession::new(&mut connection, config.clone(), state.transfer_id);
